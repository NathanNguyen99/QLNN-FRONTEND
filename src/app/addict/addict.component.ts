...
import { PlacetypeData, GenderData, ManageTypeData, Country, Ethnic, WorkStatus, Religion, Ingredient, Marriage } from "./../Shared/Services/DATA";
...
export class AddictComponent implements OnInit {
  ...
  @ViewChild('dxgridDrugs', { static: false }) dataGrid: DxDataGridComponent | undefined;
  @ViewChild('dxgridMaster', { static: false }) GridMaster: DxDataGridComponent | undefined;
  //dataSource: Addict[];
  useData: any;
  drugData: any;
  classifyData: any;
  ...
  public loadplaceOfBirth() {    
    this.service.getPlaceOfBirth().subscribe((data: any)=> {
      //console.log(data);
...
FormDetailSaved(e: any) {
  ...
  if (updValue===undefined)
    updValue=-1;
  console.log(updValue);
  this.GridMaster!.instance.cellValue(this.GridMaster!.focusedRowIndex, "updCount", updValue + 1);
  //console.log(this.EditObject);
}
  ...
  public loadAddictDrugs(key:any, odata: any) {   
    ...
    this.adrugsrv.getByAddictID(key).subscribe(data=> {
      //console.log(data);
       ...
       odata.drugs = this.addictDrugData;
      }, err => console.log(err));    
  }

  public loadAddictClassify(key:any, odata: any) {   
    //this.addictDrugData$=this.adrugsrv.getByAddictID(key);
    this.adclassifysrv.getByAddictID(key).subscribe(data=> {
      //console.log(data);
  ...
  public loadAddictPlaces(key:any, odata: any) {   
    //this.addictDrugData$=this.adrugsrv.getByAddictID(key);
    this.adplacesrv.getByAddictID(key).subscribe(data=> {
      //console.log(data);
       this.addictPlaceData = data ,
       odata.places = this.addictPlaceData;
      }, err => console.log(err));    
  }
...
getFilteredPlaces(options: any) {
  ...
  return {
      ...
      filter: options.data ? ["placeTypeID", "=", options.data.placeTypeID] : null
  };
}
...
onEditorPreparing(e: any) {  
  ...
  if (!e.row.data.managePlaceID || e.row.data.managePlaceID === "" ||
        !e.row.data.placeTypeID || e.row.data.placeTypeID === "" || e.component.hasEditData())
        this.allValid = true;
      else
        this.allValid = false;
}
...
onEditorPreparingDrug(e: any) {  
  if(e.parentType === "dataRow") {
    if (!e.row.data.drugsID || e.row.data.drugsID === "" ||
      !e.row.data.useID || e.row.data.useID === 0 || e.component.hasEditData())
      this.allValid = true;
    else
      this.allValid = false;
  }  
