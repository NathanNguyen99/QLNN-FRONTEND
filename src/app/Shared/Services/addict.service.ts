...
export class addictService extends baseService {
  ...
  getByPlace (PlaceID: string): Observable<Addict[]> {
    return this.http.get<Addict[]>(this.pathAPI + 'Addict/GetByPlaceID?placeID='+PlaceID, super.header());////pipe(catchError(super.handleError));
  }

  getByID(Oid: string): Observable<Addict> {
    return this.http.get<Addict>(this.pathAPI + `/Addict/${Oid}`, super.header());//.pipe(catchError(super.handleError));
  }
  ...
  getAllData(): void {    
    this.http.get<Addict[]>(this.pathAPI + 'Addict', super.header()).subscribe(data => {
       //console.log(data);
  ...
  getWard(): any {    
    return this.http.get(this.pathAPI + 'ManagePlace/GetByType?typeid=1', super.header());//.pipe(catchError(super.handleError));
  }

  SaveAddObject(data: any): Observable<any> {
    return this.http.put<Addict>(this.pathAPI + 'AddictDrugs', data, super.header()).pipe(
  ...
  SaveEditObject(data: any): Observable<any> {
    return this.http.post<Addict>(this.pathAPI + 'AddictDrugs', data, super.header()).pipe(
      catchError((error: any) => {
           console.error(error);
           return of();
         }),
...
getAddicts(): Observable<Addict[]> {
  this.http.get(`${this.pathAPI}Addict`, super.header()).toPromise().then((data: any) => {
      this.addict$.next(data);
  });
...
getBaseFieldAddicts(): any {
  ...
  async insert(obj: Addict): Promise<boolean> {
    ...
    const data = await this.http.post<boolean>(`${this.pathAPI}addict`, obj, super.header()).toPromise();
...
async update(obj: Addict): Promise<boolean> {
    const data = await this.http.put<boolean>(`${this.pathAPI}addict`, obj, super.header()).toPromise();
    //this.updateAddicts(change, data);
    return data;
}
 ...
 async update2(obj: Addict): Promise<boolean> {
  ...
  return await this.http.put<boolean>(`${this.pathAPI}addict`, obj, super.header()).toPromise();//.pipe(catchError(super.handleError)).toPromise(); 
}
...
async remove(obj: Addict): Promise<boolean> {
    ...
    const data = await this.http.delete<boolean>(`${this.pathAPI}addict/${obj.oid}`, super.header()).toPromise();
...
async saveObjChange(obj: Addict, type: string) : Promise<Addict> {
  ...
  if (type==="insert")
    return this.insert3(obj);

    if (type==="update")
      return this.update3(obj);
    else
      return this.remove3(obj);
...
const data= await this.http.put<boolean>(`${this.pathAPI}addict`, obj, super.header());//.pipe(catchError(super.handleError)).toPromise(); 
return obj;
}
...
UploadImage(formdata: any): Observable<Addict[]> {
  ...
  const data = this.http.post(`${this.pathAPI}addict/CheckFace`, formdata, super.header3());
